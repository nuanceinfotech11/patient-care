...
use App\Models\{User,Role,Patient_schedule,Inventory,Stockin};
...
class StockInController extends Controller
{
    public function index(Request $request)
    {
       //echo"index stock";die;
        ...
        $listUrl='stockin-list';
        if($userType=="Admin")
        {
            $listUrl = 'admin-stockin-list';
        }
        $deleteUrl = 'company-admin-delete';
        if($userType=="Admin")
        {
            $deleteUrl = 'admin-stockin-delete';
        }
        $perpage = config('app.perpage');
        $breadcrumbs = [
            ['link' => "modern", 'name' => "Home"], ['link' => "javascript:void(0)", 'name' => __('locale.stock_in')], ['name' => __('locale.stock_in_list')]];
        ...
        if($request->ajax()){
            // $patientscheduleResult = $patientscheduleResult->when($request->seach_term, function($q)use($request){
                ...
                // })->paginate($perpage);
                $stockResult = $stockResult->whereHas('inventoryname', function($q)use($request){
                    $q->where('id', 'like', '%'.$request->seach_term.'%')
    ...
    {
        //echo"stock create";die;
        //echo auth()->user()->id;die;
        $userType = auth()->user()->role()->first()->name;
        $formUrl = 'company-admin-create';
        $stockin_result=$states=$cities=false;
        $breadcrumbs = [
        ...
        if($id!=''){
            //$permission_arr = [];
            $stockin_result = Stockin::find($id);
           ...
           if($userType=="superadmin")
            ...
            if($userType=="Admin")
            {
            $formUrl = 'admin-stockin-update';
            }
        }
        //dd($patient_schedule_result);
        return view('pages.stockin.stockin-create', ['pageConfigs' => $pageConfigs], ['breadcrumbs' => $breadcrumbs,'patient'=>$patient,'pageTitle'=>$pageTitle,'inventory'=>$inventory,'stockin_result'=>$stockin_result,'states'=>$states,'cities'=>$cities,'userType'=>$userType,'formUrl'=>$formUrl,'carer'=>$carer,'roles'=>$roles]);
    }
    ...
    public function store(Request $request){
        ...
        {
            $backUrl='stockin-list';
        }
        if(auth()->user()->role()->first()->name=="Admin")
        {
            $backUrl='admin-stockin-list';
        }
     ...
     public function update(Request $request, $id)
    {
        ...
        $stockinData = Stockin::where('id',$id)->update($request->all());
        // dd($request->all());
        // $MappingData = [];
