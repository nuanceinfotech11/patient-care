    ...
    <div class="card">
      ...
      @include('panels.flashMessages')
        <!-- datatable start -->
          ...
          <div class="col s12 m6 l6 add-btn" style="text-align:end;">
                  ...
                  @if(auth()->user()->role()->first()->name=="superadmin")
                    <a href="{{route('patient-carer-map-create')}}">
                  @endif
                  @if(auth()->user()->role()->first()->name=="Admin")
                    <a href="{{route('admin-patient-carer-map-create')}}">
                  @endif
                  @if(auth()->user()->role()->first()->name=="Manager")
                    <a href="{{route('manager-patient-carer-map-create')}}">
                  @endif
        ...
        <div class="responsive-table table-result">
          @include('pages.patient-carer-map.patient-carer-map-list-ajax')
...
<!-- {{(isset(auth()->user()->role()->first()->name) && auth()->user()->role()->first()->name=="superadmin") ? route("patient-carer-map-list") : route("admin-patient-carer-map-list") }} -->
...
<script>
  $(document).ready(function(){
    // var paginationUrl = '{{(isset(auth()->user()->role()->first()->name) && auth()->user()->role()->first()->name=="superadmin") ? route("patient-carer-map-list") : route("admin-patient-carer-map-list") }}';
    var paginationUrl='{{$listUrl}}';
