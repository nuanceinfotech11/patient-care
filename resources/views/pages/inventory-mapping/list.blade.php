    ...
    <div class="card">
      ...
      @include('panels.flashMessages')
        <!-- datatable start -->
          ...
          <div class="col s12 m6 l6 add-btn" style="text-align:end;">
                  ...
                  @if(auth()->user()->role()->first()->name=="superadmin")
                    <a href="{{route('inventory-mapping.create')}}">
                  @endif
                  @if(auth()->user()->role()->first()->name=="Admin")
                    <a href="{{route('admin-inventory-mapping-create')}}">
                  @endif
        ...
        <div class="responsive-table table-result">
          @include('pages.inventory-mapping.ajax-list')
...
<script>
  $(document).ready(function(){
   // var paginationUrl = '{{(isset($paginationUrl) && $paginationUrl!='') ? route($paginationUrl) : route("inventory-mapping.index") }}';
   var paginationUrl = '{{(isset(auth()->user()->role()->first()->name) && auth()->user()->role()->first()->name=="superadmin") ? route("inventory-mapping.index") : route("admin-inventory-mapping-list") }}';
